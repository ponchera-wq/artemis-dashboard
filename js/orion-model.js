...
//
// This is V1 geometry (proven working) with gold ESM foil.
// All userData exports required by trajectory.js are included.
...
function createOrionModel(THREE) {
  ...
  // Docking ring
  var dockRing = new THREE.Mesh(new THREE.TorusGeometry(0.035, 0.006, 8, 16), frameMat);
  ...
  // Crew Module (bell/gumdrop LatheGeometry)
  var cmProfile = [];
  for (var i = 0; i <= 20; i++) {
    var t = i / 20;
  ...
  // ESM (gold foil — matches GLB Thermal_Gold material)
  var esm = new THREE.Mesh(new THREE.CylinderGeometry(0.108, 0.108, 0.22, 24), goldMLI);
  ...
  // ── 4 Solar Array Wings (V1 proven X-pattern layout) ──────────────────
